...
public final class Box extends AggregateRoot<BoxId> {
    ...
    private Box(BoxId boxId, UUID createdBy, TxRef txRef, BoxState state, Battery batteryLevel, WeightLimit weightLimit, Weight boxWeight) {
        super(boxId, createdBy);
        this.txRef = txRef;
        this.state = state;
        this.weightLimit = weightLimit;
        this.batteryLevel = batteryLevel;
        totalItemsWeight = boxWeight;
    }
    public static Result<Box, DomainError> create(String rawTxRef, double weightLimitGrams, double batteryPercentage, UUID createdBy) {
        ...
        Result<Weight, DomainError> boxWeightResult = Weight.of(BoxConstants.BOX_WEIGHT * 1000);
        if (boxWeightResult.isFailure()) {
            return  Result.failure(boxWeightResult.getError());
        }
    ...
    public static Result<Box, DomainError> reconstitute(
        ...
        return Result.success(new Box(id, createdBy, txRef, state, batteryLevel, weightLimit, weightResult.getValue()));
    }
